...
import { ArrowLeft, Volume2, VolumeX, Navigation, MapPin, ChevronUp, ChevronDown, CheckCircle2, Clock } from "lucide-react";
...
const NavigationView = () => {
  ...
  useEffect(() => {
    if (!circuit) return;
    // Simulate arriving at a stop - auto-play audio
  ...
  return (
    <div className="h-screen flex flex-col bg-background relative">
      ...
      <div className="flex-1 relative">
        ...
        {/* Audio indicator */}
        <AnimatePresence>
          {audioPlaying && currentStop?.audioText && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 20 }}
              className="absolute bottom-4 left-4 right-4 z-[1000] glass-card shadow-elevated rounded-2xl p-4"
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-foreground leading-relaxed">{currentStop.audioText}</p>
      ...
      <div className="relative z-[1000]">
        <button
          ...
          className="w-full flex items-center justify-center py-2 bg-card border-t border-border"
        >
        ...
        <div className="bg-card border-t border-border px-4 pb-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 py-3 rounded-xl bg-muted text-foreground font-medium text-sm disabled:opacity-30"
            >
              Précédent
            </button>
            <button
              onClick={() => {
                toggleVisited(currentStop.id);
                setCurrentStopIndex(Math.min(circuit.stops.length - 1, currentStopIndex + 1));
              }}
              className="flex-1 py-3 rounded-xl bg-gradient-hero text-primary-foreground font-medium text-sm"
            >
              Suivant →
            </button>
          </div>
                ...
                {circuit.stops.map((stop, i) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 p-3 rounded-xl text-left transition-colors ${
                      i === currentStopIndex ? "bg-primary/10" : "hover:bg-muted"
                    }`}
                  >
                    <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center text-sm font-medium text-muted-foreground">
                      {visitedStops.has(stop.id) ? (
                    ...
                    {stop.audioText && <Volume2 className="w-4 h-4 text-secondary flex-shrink-0" />}
                  </button>
                ))}
              </div>
            </motion.div>
